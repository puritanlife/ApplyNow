...
CREATE procedure [dbo].[spGetPhoneType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the Phone Type table. 
**************************************************/
	
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetPhoneType]' 
	...
	SELECT 
	  tblT.[PhoneTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblPhoneType] tblT with(nolock) 
	where isnull(tblT.[PhoneTypeID],'') = isnull(@TypeID,tblT.[PhoneTypeID])
