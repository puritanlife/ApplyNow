...
CREATE procedure [dbo].[spGetPlanType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the Plan Type table. 
**************************************************/

	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetPlanType]' 
	...
	SELECT 
	  tblT.[PlanTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblPlanType] tblT with(nolock) 
	where isnull(tblT.[PlanTypeID],'') = isnull(@TypeID,tblT.[PlanTypeID])
