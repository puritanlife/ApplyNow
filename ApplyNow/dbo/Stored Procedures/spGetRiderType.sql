...
CREATE procedure [dbo].[spGetRiderType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the Rider Type table. 
**************************************************/

	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetRiderType]' 
	...
	SELECT 
	  tblT.[RiderTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblRiderType] tblT with(nolock) 
	where isnull(tblT.[RiderTypeID],'') = isnull(@TypeID,tblT.[RiderTypeID])
