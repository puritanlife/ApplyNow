...
CREATE procedure [dbo].[spGetAddressType]
(
    @TypeID [int] Null,
...
**************************************************/
	...
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetAddressType]' 
	...
	SET @Parameters = '@TypeID ' + cast(@TypeID as nvarchar) + 
	                  ' @Type ' + cast(@Type as nvarchar) + 
					  ' @Descr ' + cast(@Descr as nvarchar)

	SELECT 
	  tblT.[AddressTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblAddressType] tblT with(nolock) 
	where isnull(tblT.[AddressTypeID],'') = isnull(@TypeID,tblT.[AddressTypeID])
