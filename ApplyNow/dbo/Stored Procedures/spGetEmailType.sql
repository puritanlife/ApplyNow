...
CREATE procedure [dbo].[spGetEmailType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the Email Type table. 
**************************************************/
		
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetEmailType]' 
	...
	DECLARE @Parameters nvarchar(max) = '@TypeID ' + cast(@TypeID as nvarchar) + ' @Type ' + cast(@Type as nvarchar) + ' @Descr ' + cast(@Descr as nvarchar)

	SELECT 
	  tblT.[EmailTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblEmailType] tblT with(nolock) 
	where isnull(tblT.[EmailTypeID],'') = isnull(@TypeID,tblT.[EmailTypeID])
