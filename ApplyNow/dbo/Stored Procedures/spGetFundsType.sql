...
CREATE procedure [dbo].[spGetFundsType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Create Date: 5/31/2020
  Description: Returns all data within the Funds Type table. 
**************************************************/
	
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetFundsType]' 
	...
	SELECT 
	  tblT.[FundsTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblFundsType] tblT with(nolock) 
	where isnull(tblT.[FundsTypeID],'') = isnull(@TypeID,tblT.[FundsTypeID])
