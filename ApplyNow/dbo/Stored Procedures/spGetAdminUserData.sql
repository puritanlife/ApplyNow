...
CREATE procedure [dbo].[spGetAdminUserData]
(
...
-- Description: Obtains Admin User (tblAdminUser) information.
*************************************************/
	
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetAdminUserData]' 
	...
	FROM [dbo].[tblAdminUser] AS u with(nolock)
	WHERE u.[AdminUserId] = ISNULL(@AdminUserID, u.[AdminUserId])
