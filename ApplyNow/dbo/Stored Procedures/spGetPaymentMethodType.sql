...
CREATE procedure [dbo].[spGetPaymentMethodType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the Payment Method Type table. 
**************************************************/
	
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetPaymentMethodType]' 
	...
	SELECT 
	  tblT.[PaymentMethodTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblPaymentMethodType] tblT with(nolock) 
	where isnull(tblT.[PaymentMethodTypeID],'') = isnull(@TypeID,tblT.[PaymentMethodTypeID])
