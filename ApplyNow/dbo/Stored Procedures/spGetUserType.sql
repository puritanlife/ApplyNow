...
/*************************************************
  ...
  Description: Returns all data within the User Type table. 
**************************************************/
	...
	SELECT 
	  tblT.[UserTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblUserType] tblT with(nolock) 
	where isnull(tblT.[UserTypeID],'') = isnull(@TypeID,tblT.[UserTypeID])
