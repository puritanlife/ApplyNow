...
CREATE procedure [dbo].[spGetStatusType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the Status Type table. 
**************************************************/

	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetStatusType]' 
	...
	SELECT 
	  tblT.[StatusTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblStatusType] tblT with(nolock) 
	where isnull(tblT.[StatusTypeID],'') = isnull(@TypeID,tblT.[StatusTypeID])
