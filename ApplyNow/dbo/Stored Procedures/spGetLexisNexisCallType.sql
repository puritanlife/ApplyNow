...
CREATE procedure [dbo].[spGetLexisNexisCallType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the Lexis Nexis Call Type table. 
**************************************************/
	
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetLexisNexisCallType]' 
	...
	SELECT [LexisNexisCallTypeID]
		  ,[Type]
		  ,[Descr]
		  ,[CreatedDate]
		  ,[LastModifiedDate]
	FROM [dbo].[tblLexisNexisCallType] tblT with(nolock) 
	where isnull(tblT.[LexisNexisCallTypeID],'') = isnull(@TypeID,tblT.[LexisNexisCallTypeID])
