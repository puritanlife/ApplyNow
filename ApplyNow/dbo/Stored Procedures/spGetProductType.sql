...
/*************************************************
  ...
  Description: Returns all data within the Product Type table. 
**************************************************/
	
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetProductType]' 
	...
	SELECT 
	  tblT.[ProductTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblProductType] tblT with(nolock) 
	where isnull(tblT.[ProductTypeID],'') = isnull(@TypeID,tblT.[ProductTypeID])
