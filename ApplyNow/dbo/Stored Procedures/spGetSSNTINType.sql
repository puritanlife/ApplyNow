...
CREATE procedure [dbo].[spGetSSNTINType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the SSNTIN Type table. 
**************************************************/

	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetSSNTINType]' 
	...
	SELECT 
	  tblT.[SSNTINTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblSSNTINType] tblT with(nolock) 
	where isnull(tblT.[SSNTINTypeID],'') = isnull(@TypeID,tblT.[SSNTINTypeID])
