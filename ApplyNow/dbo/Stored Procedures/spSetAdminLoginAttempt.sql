...
CREATE procedure [dbo].[spSetAdminLoginAttempt]
(
...
/*************************************************
	...
	Description: Inserts a record into tblAdmingLoginAttempt table. 
*************************************************/
	
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spSetAdminLoginAttempt]' 
	...
	SET @count = (SELECT COUNT(*) FROM dbo.tblAdminUser u with(nolock) WHERE u.Email = @Email)
	
    SET @AccountStatus = (CASE WHEN @count > 0 
	                           THEN (SELECT u.locked FROM dbo.tblAdminUser u with(nolock) WHERE u.Email = @Email) 
			                   ELSE 2 END)

	SET @Parameters = '@Email ' + cast(@Email as nvarchar) + ' @count ' + cast(@count as nvarchar) + ' @AccountStatus ' + cast(@AccountStatus as nvarchar)

	BEGIN
		BEGIN TRY

			INSERT INTO [dbo].[tblAdminLoginAttempt]
				([AdminUserID]
				,[Email]
				,[SuccessFlag]
				,[AttemptDate]
				,[AttemptMessage])
			(SELECT (SELECT u.AdminUserID FROM dbo.tblAdminUser u with(nolock) WHERE u.Email = @Email) AS [UserID]
			, @Email                                                            AS [Email]
