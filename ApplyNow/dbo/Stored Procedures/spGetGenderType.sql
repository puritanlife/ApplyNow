...
CREATE procedure [dbo].[spGetGenderType]
(
    @TypeID [int] Null,
...
/*************************************************
  ...
  Description: Returns all data within the Gender Type table. 
**************************************************/
	
	-- Declaring variables for try and catch error handling
	SET CONCAT_NULL_YIELDS_NULL OFF;
	DECLARE @StartSPCall datetime = getdate()
	DECLARE @Procedure nvarchar(max) = '[dbo].[spGetGenderType]' 
	...
	SELECT 
	  tblT.[GenderTypeID] as [TypeID]
	, tblT.[Type] as [Type]
	, tblT.[Descr] as [Descr]
	FROM [dbo].[tblGenderType] tblT with(nolock) 
	where isnull(tblT.[GenderTypeID],'') = isnull(@TypeID,tblT.[GenderTypeID])
