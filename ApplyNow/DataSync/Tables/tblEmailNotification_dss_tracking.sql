CREATE TABLE [DataSync].[tblEmailNotification_dss_tracking] (
    ...
    CONSTRAINT [PK_DataSync.tblEmailNotification_dss_tracking] PRIMARY KEY CLUSTERED ([EmailNotificationID] ASC)
);


GO
CREATE NONCLUSTERED INDEX [tombstone_index]
    ON [DataSync].[tblEmailNotification_dss_tracking]([sync_row_is_tombstone] ASC, [local_update_peer_timestamp] ASC)
    INCLUDE([last_change_datetime]);


GO
CREATE NONCLUSTERED INDEX [local_update_peer_timestamp_index]
    ON [DataSync].[tblEmailNotification_dss_tracking]([local_update_peer_timestamp] ASC, [EmailNotificationID] ASC);
