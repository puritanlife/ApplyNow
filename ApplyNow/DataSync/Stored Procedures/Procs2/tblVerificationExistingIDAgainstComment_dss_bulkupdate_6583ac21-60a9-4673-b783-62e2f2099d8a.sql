CREATE PROCEDURE [DataSync].[tblVerificationExistingIDAgainstComment_dss_bulkupdate_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [DataSync].[tblVerificationExistingIDAgainstComment_dss_BulkType_6583ac21-60a9-4673-b783-62e2f2099d8a] READONLY
AS
BEGIN
...
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [DataSync].[tblVerificationExistingIDAgainstComment_dss_tracking] t ON p.[LogExistingIDAgainstCommentID] = t.[LogExistingIDAgainstCommentID]) as changes ON changes.[LogExistingIDAgainstCommentID] = base.[LogExistingIDAgainstCommentID]
...
OR (changes.update_scope_local_id IS NULL AND changes.scope_update_peer_key IS NULL AND changes.local_update_peer_timestamp IS NULL) 
 THEN
UPDATE SET [LogVerificationCommentSQLID] = changes.[LogVerificationCommentSQLID], [ExistingID] = changes.[ExistingID], [AsOfDate] = changes.[AsOfDate], [CreatedDate] = changes.[CreatedDate], [LastModifiedDate] = changes.[LastModifiedDate]
