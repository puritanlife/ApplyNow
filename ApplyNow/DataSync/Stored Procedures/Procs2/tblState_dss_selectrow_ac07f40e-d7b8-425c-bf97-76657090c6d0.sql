CREATE PROCEDURE [DataSync].[tblState_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[StateID]
ELSE [base].[StateID] END) 
as [StateID], 
[base].[StateShort], [base].[StateLong], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblState] WHERE [StateID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblState_dss_tracking] WHERE [StateID] = @P_1) [side] ON [base].[StateID] = [side].[StateID]
END