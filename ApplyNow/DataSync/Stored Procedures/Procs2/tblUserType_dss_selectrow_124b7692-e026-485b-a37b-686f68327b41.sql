CREATE PROCEDURE [DataSync].[tblUserType_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[UserTypeID]
ELSE [base].[UserTypeID] END) 
as [UserTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblUserType] WHERE [UserTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblUserType_dss_tracking] WHERE [UserTypeID] = @P_1) [side] ON [base].[UserTypeID] = [side].[UserTypeID]
END