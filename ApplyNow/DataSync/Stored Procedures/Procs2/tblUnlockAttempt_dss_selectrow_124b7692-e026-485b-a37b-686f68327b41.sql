CREATE PROCEDURE [DataSync].[tblUnlockAttempt_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblUnlockAttempt] WHERE [UnlockAttemptID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblUnlockAttempt_dss_tracking] WHERE [UnlockAttemptID] = @P_1) [side] ON [base].[UnlockAttemptID] = [side].[UnlockAttemptID]
END