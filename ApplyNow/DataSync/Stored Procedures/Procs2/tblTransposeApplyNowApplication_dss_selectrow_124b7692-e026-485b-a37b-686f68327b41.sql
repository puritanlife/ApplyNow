CREATE PROCEDURE [DataSync].[tblTransposeApplyNowApplication_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[TransposeApplyNowApplicationID]
ELSE [base].[TransposeApplyNowApplicationID] END) 
as [TransposeApplyNowApplicationID], 
[base].[ApplicationID], [base].[ApplyNowIDOwner], [base].[ApplyNowIDJointOwner], [base].[ApplyNowIDAnnuitant], [base].[ApplyNowIDJointAnnuitant], [base].[ApplyNowIDBeneficiary1], [base].[ApplyNowIDBeneficiary2], [base].[ApplyNowIDBeneficiary3], [base].[ApplyNowIDBeneficiary4], [base].[ApplyNowIDBeneficiary5], [base].[ApplyNowIDReplacementCompany], [base].[ApplyNowIDUnknown], [base].[ApplyNowIDAgent], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblTransposeApplyNowApplication] WHERE [TransposeApplyNowApplicationID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblTransposeApplyNowApplication_dss_tracking] WHERE [TransposeApplyNowApplicationID] = @P_1) [side] ON [base].[TransposeApplyNowApplicationID] = [side].[TransposeApplyNowApplicationID]
END