CREATE PROCEDURE [DataSync].[tblVerificationExistingIDAgainstComment_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[LogExistingIDAgainstCommentID]
ELSE [base].[LogExistingIDAgainstCommentID] END) 
as [LogExistingIDAgainstCommentID], 
[base].[LogVerificationCommentSQLID], [base].[ExistingID], [base].[AsOfDate], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [maint].[tblVerificationExistingIDAgainstComment] WHERE [LogExistingIDAgainstCommentID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblVerificationExistingIDAgainstComment_dss_tracking] WHERE [LogExistingIDAgainstCommentID] = @P_1) [side] ON [base].[LogExistingIDAgainstCommentID] = [side].[LogExistingIDAgainstCommentID]
END