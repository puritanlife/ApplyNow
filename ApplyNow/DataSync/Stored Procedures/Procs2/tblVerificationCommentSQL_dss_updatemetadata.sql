CREATE PROCEDURE [DataSync].[tblVerificationCommentSQL_dss_updatemetadata]
	@P_1 Int,
...
USING (SELECT [LogVerificationCommentSQLID]
 FROM [maint].[tblVerificationCommentSQL]
 WHERE [LogVerificationCommentSQLID] = @P_1
) AS source([LogVerificationCommentSQLID])
...
VALUES (
[source].[LogVerificationCommentSQLID],NULL, @marker_scope_create_peer_key, @marker_scope_create_peer_timestamp, 0, @marker_local_create_peer_timestamp , @sync_scope_local_id, @sync_update_peer_key, @sync_update_peer_timestamp, 0, 0, GETDATE() );

SET @sync_row_count = @@ROWCOUNT
IF @sync_row_count = 0 
BEGIN
UPDATE [DataSync].[tblVerificationCommentSQL_dss_tracking] SET [update_scope_local_id] = @sync_scope_local_id, [scope_update_peer_key] = @sync_update_peer_key, [scope_update_peer_timestamp] = @sync_update_peer_timestamp, [local_update_peer_key] = 0, [sync_row_is_tombstone] = @sync_row_is_tombstone WHERE ([LogVerificationCommentSQLID] = @P_1) AND (@sync_check_concurrency = 0 or [local_update_peer_timestamp] = @sync_row_timestamp);SET @sync_row_count = @@ROWCOUNT;
END
 END;
END