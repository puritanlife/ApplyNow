CREATE PROCEDURE [DataSync].[tblStateLicenseIndictor_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[StateLicenseIndictorID]
ELSE [base].[StateLicenseIndictorID] END) 
as [StateLicenseIndictorID], 
[base].[StateID], [base].[StateLicenseTypeID], [base].[EffectiveDate], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblStateLicenseIndictor] WHERE [StateLicenseIndictorID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblStateLicenseIndictor_dss_tracking] WHERE [StateLicenseIndictorID] = @P_1) [side] ON [base].[StateLicenseIndictorID] = [side].[StateLicenseIndictorID]
END