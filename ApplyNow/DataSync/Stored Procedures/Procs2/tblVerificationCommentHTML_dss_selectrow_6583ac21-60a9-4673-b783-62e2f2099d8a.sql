CREATE PROCEDURE [DataSync].[tblVerificationCommentHTML_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [maint].[tblVerificationCommentHTML] WHERE [LogVerificationCommentHTMLID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblVerificationCommentHTML_dss_tracking] WHERE [LogVerificationCommentHTMLID] = @P_1) [side] ON [base].[LogVerificationCommentHTMLID] = [side].[LogVerificationCommentHTMLID]
END