CREATE PROCEDURE [DataSync].[tblUser_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[UserId]
ELSE [base].[UserId] END) 
as [UserId], 
[base].[PersonID], [base].[UserTypeID], [base].[Email], [base].[Pass], [base].[Attempt], [base].[locked], [base].[Last_Login], [base].[IsAdmin], [base].[PasswordChangeDate], [base].[Question1], [base].[Answer1], [base].[Question2], [base].[Answer2], [base].[Question3], [base].[Answer3], [base].[Question1ChangeDate], [base].[Answer1ChangeDate], [base].[Question2ChangeDate], [base].[Answer2ChangeDate], [base].[Question3ChangeDate], [base].[Answer3ChangeDate], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblUser] WHERE [UserId] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblUser_dss_tracking] WHERE [UserId] = @P_1) [side] ON [base].[UserId] = [side].[UserId]
END