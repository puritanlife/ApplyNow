CREATE PROCEDURE [DataSync].[tblStateLicenseType_dss_selectchanges_124b7692-e026-485b-a37b-686f68327b41]
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM [dbo].[tblStateLicenseType] [base] FULL OUTER JOIN [DataSync].[tblStateLicenseType_dss_tracking] [side] ON [base].[StateLicenseTypeID] = [side].[StateLicenseTypeID]
 WHERE 
