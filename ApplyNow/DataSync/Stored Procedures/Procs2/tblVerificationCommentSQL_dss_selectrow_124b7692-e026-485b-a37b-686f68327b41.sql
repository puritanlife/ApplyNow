CREATE PROCEDURE [DataSync].[tblVerificationCommentSQL_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[LogVerificationCommentSQLID]
ELSE [base].[LogVerificationCommentSQLID] END) 
as [LogVerificationCommentSQLID], 
[base].[Comment], [base].[SQL], [base].[CreatedDate], [base].[LastModifiedDate], [base].[StatusTypeID], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [maint].[tblVerificationCommentSQL] WHERE [LogVerificationCommentSQLID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblVerificationCommentSQL_dss_tracking] WHERE [LogVerificationCommentSQLID] = @P_1) [side] ON [base].[LogVerificationCommentSQLID] = [side].[LogVerificationCommentSQLID]
END