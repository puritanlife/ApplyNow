CREATE PROCEDURE [DataSync].[tblStatusType_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[StatusTypeID]
ELSE [base].[StatusTypeID] END) 
as [StatusTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblStatusType] WHERE [StatusTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblStatusType_dss_tracking] WHERE [StatusTypeID] = @P_1) [side] ON [base].[StatusTypeID] = [side].[StatusTypeID]
END