CREATE PROCEDURE [DataSync].[tblUser_dss_bulkdelete_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [DataSync].[tblUser_dss_BulkType_ac07f40e-d7b8-425c-bf97-76657090c6d0] READONLY
AS
BEGIN
...
DELETE [dbo].[tblUser] 
OUTPUT DELETED.[UserId] INTO @changed FROM [dbo].[tblUser] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblUser_dss_tracking] t ON p.[UserId] = t.[UserId]) as changes ON changes.[UserId] = base.[UserId] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
