CREATE PROCEDURE [DataSync].[tblProductPlanPeriod_dss_bulkupdate_124b7692-e026-485b-a37b-686f68327b41]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [DataSync].[tblProductPlanPeriod_dss_BulkType_124b7692-e026-485b-a37b-686f68327b41] READONLY
AS
BEGIN
...
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [DataSync].[tblProductPlanPeriod_dss_tracking] t ON p.[ProductPlanPeriodID] = t.[ProductPlanPeriodID]) as changes ON changes.[ProductPlanPeriodID] = base.[ProductPlanPeriodID]
...
OR (changes.update_scope_local_id IS NULL AND changes.scope_update_peer_key IS NULL AND changes.local_update_peer_timestamp IS NULL) 
 THEN
UPDATE SET [ProductTypeID] = changes.[ProductTypeID], [PlanTypeID] = changes.[PlanTypeID], [PeriodTypeID] = changes.[PeriodTypeID], [PremMin] = changes.[PremMin], [PremMax] = changes.[PremMax], [CreditingRate] = changes.[CreditingRate], [EffectiveDate] = changes.[EffectiveDate], [EndDate] = changes.[EndDate], [CreatedDate] = changes.[CreatedDate], [LastModifiedDate] = changes.[LastModifiedDate]
