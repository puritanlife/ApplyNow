CREATE PROCEDURE [DataSync].[tblLoginAttempt_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[LoginAttemptID]
ELSE [base].[LoginAttemptID] END) 
as [LoginAttemptID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblLoginAttempt] WHERE [LoginAttemptID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblLoginAttempt_dss_tracking] WHERE [LoginAttemptID] = @P_1) [side] ON [base].[LoginAttemptID] = [side].[LoginAttemptID]
END