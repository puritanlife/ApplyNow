CREATE PROCEDURE [DataSync].[tblLeadsNonLicenseStates_dss_bulkdelete_124b7692-e026-485b-a37b-686f68327b41]
...
DELETE [dbo].[tblLeadsNonLicenseStates] 
OUTPUT DELETED.[LeadsNonLicenseStatesID] INTO @changed FROM [dbo].[tblLeadsNonLicenseStates] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblLeadsNonLicenseStates_dss_tracking] t ON p.[LeadsNonLicenseStatesID] = t.[LeadsNonLicenseStatesID]) as changes ON changes.[LeadsNonLicenseStatesID] = base.[LeadsNonLicenseStatesID] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
