CREATE PROCEDURE [DataSync].[tblApplyNow_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[ApplyNowID]
ELSE [base].[ApplyNowID] END) 
as [ApplyNowID], 
[base].[ApplicationID], [base].[PersonID], [base].[PersonTypeID], [base].[Percentage], [base].[RelationshipTypeID], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblApplyNow] WHERE [ApplyNowID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblApplyNow_dss_tracking] WHERE [ApplyNowID] = @P_1) [side] ON [base].[ApplyNowID] = [side].[ApplyNowID]
END