CREATE PROCEDURE [DataSync].[tblEmailNotification_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[EmailNotificationID]
ELSE [base].[EmailNotificationID] END) 
as [EmailNotificationID], 
[base].[EmailAddress], [base].[SG_MessageID], [base].[SG_EventID], [base].[EventStatus], [base].[IP_Address], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblEmailNotification] WHERE [EmailNotificationID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblEmailNotification_dss_tracking] WHERE [EmailNotificationID] = @P_1) [side] ON [base].[EmailNotificationID] = [side].[EmailNotificationID]
END