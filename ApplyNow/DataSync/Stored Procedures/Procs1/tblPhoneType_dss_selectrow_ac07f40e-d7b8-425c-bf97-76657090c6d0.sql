CREATE PROCEDURE [DataSync].[tblPhoneType_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[PhoneTypeID]
ELSE [base].[PhoneTypeID] END) 
as [PhoneTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPhoneType] WHERE [PhoneTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPhoneType_dss_tracking] WHERE [PhoneTypeID] = @P_1) [side] ON [base].[PhoneTypeID] = [side].[PhoneTypeID]
END