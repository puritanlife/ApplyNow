CREATE PROCEDURE [DataSync].[tblApplication_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[ApplicationID]
ELSE [base].[ApplicationID] END) 
as [ApplicationID], 
[base].[ProductPlanPeriodID], [base].[UUID], [base].[StateID], [base].[Premium], [base].[ForCompany], [base].[StatusTypeID], [base].[CanvasPolicyNumber], [base].[CreatedDate], [base].[LastModifiedDate], [base].[StateLicenseTypeID], [base].[AppValidation], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblApplication] WHERE [ApplicationID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblApplication_dss_tracking] WHERE [ApplicationID] = @P_1) [side] ON [base].[ApplicationID] = [side].[ApplicationID]
END