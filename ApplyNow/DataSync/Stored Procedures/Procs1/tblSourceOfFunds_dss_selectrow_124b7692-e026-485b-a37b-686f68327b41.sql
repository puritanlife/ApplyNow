CREATE PROCEDURE [DataSync].[tblSourceOfFunds_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[SourceOfFundsID]
ELSE [base].[SourceOfFundsID] END) 
as [SourceOfFundsID], 
[base].[ApplicationID], [base].[FundsTypeID], [base].[Contribution], [base].[ContributionYear], [base].[Transfer], [base].[Rollover], [base].[Conversion], [base].[ConversionYear], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblSourceOfFunds] WHERE [SourceOfFundsID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblSourceOfFunds_dss_tracking] WHERE [SourceOfFundsID] = @P_1) [side] ON [base].[SourceOfFundsID] = [side].[SourceOfFundsID]
END