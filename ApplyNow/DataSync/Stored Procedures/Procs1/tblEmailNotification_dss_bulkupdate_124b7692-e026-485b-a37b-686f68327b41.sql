CREATE PROCEDURE [DataSync].[tblEmailNotification_dss_bulkupdate_124b7692-e026-485b-a37b-686f68327b41]
...
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [DataSync].[tblEmailNotification_dss_tracking] t ON p.[EmailNotificationID] = t.[EmailNotificationID]) as changes ON changes.[EmailNotificationID] = base.[EmailNotificationID]
...
OR (changes.update_scope_local_id IS NULL AND changes.scope_update_peer_key IS NULL AND changes.local_update_peer_timestamp IS NULL) 
 THEN
UPDATE SET [EmailAddress] = changes.[EmailAddress], [SG_MessageID] = changes.[SG_MessageID], [SG_EventID] = changes.[SG_EventID], [EventStatus] = changes.[EventStatus], [IP_Address] = changes.[IP_Address], [CreatedDate] = changes.[CreatedDate], [LastModifiedDate] = changes.[LastModifiedDate]
