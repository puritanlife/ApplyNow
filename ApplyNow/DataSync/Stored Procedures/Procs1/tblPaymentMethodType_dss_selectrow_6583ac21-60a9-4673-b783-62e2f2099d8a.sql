CREATE PROCEDURE [DataSync].[tblPaymentMethodType_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[PaymentMethodTypeID]
ELSE [base].[PaymentMethodTypeID] END) 
as [PaymentMethodTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPaymentMethodType] WHERE [PaymentMethodTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPaymentMethodType_dss_tracking] WHERE [PaymentMethodTypeID] = @P_1) [side] ON [base].[PaymentMethodTypeID] = [side].[PaymentMethodTypeID]
END