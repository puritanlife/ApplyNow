CREATE PROCEDURE [DataSync].[tblPersonType_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[PersonTypeID]
ELSE [base].[PersonTypeID] END) 
as [PersonTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPersonType] WHERE [PersonTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPersonType_dss_tracking] WHERE [PersonTypeID] = @P_1) [side] ON [base].[PersonTypeID] = [side].[PersonTypeID]
END