CREATE PROCEDURE [DataSync].[tblLexisNexisCallType_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[LexisNexisCallTypeID]
ELSE [base].[LexisNexisCallTypeID] END) 
as [LexisNexisCallTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblLexisNexisCallType] WHERE [LexisNexisCallTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblLexisNexisCallType_dss_tracking] WHERE [LexisNexisCallTypeID] = @P_1) [side] ON [base].[LexisNexisCallTypeID] = [side].[LexisNexisCallTypeID]
END