CREATE PROCEDURE [DataSync].[tblGenderType_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[GenderTypeID]
ELSE [base].[GenderTypeID] END) 
as [GenderTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblGenderType] WHERE [GenderTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblGenderType_dss_tracking] WHERE [GenderTypeID] = @P_1) [side] ON [base].[GenderTypeID] = [side].[GenderTypeID]
END