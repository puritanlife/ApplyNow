CREATE PROCEDURE [DataSync].[tblAdminUser_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[AdminUserId]
ELSE [base].[AdminUserId] END) 
as [AdminUserId], 
[base].[Email], [base].[Pass], [base].[Attempt], [base].[locked], [base].[Last_Login], [base].[PasswordChangeDate], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblAdminUser] WHERE [AdminUserId] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblAdminUser_dss_tracking] WHERE [AdminUserId] = @P_1) [side] ON [base].[AdminUserId] = [side].[AdminUserId]
END