CREATE PROCEDURE [DataSync].[tblFundsType_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[FundsTypeID]
ELSE [base].[FundsTypeID] END) 
as [FundsTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblFundsType] WHERE [FundsTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblFundsType_dss_tracking] WHERE [FundsTypeID] = @P_1) [side] ON [base].[FundsTypeID] = [side].[FundsTypeID]
END