CREATE PROCEDURE [DataSync].[tblPasswordReset_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[PasswordResetId]
ELSE [base].[PasswordResetId] END) 
as [PasswordResetId], 
[base].[UserId], [base].[Request_Date], [base].[PasswordUsedDate], [base].[PasswordUsed], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPasswordReset] WHERE [PasswordResetId] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPasswordReset_dss_tracking] WHERE [PasswordResetId] = @P_1) [side] ON [base].[PasswordResetId] = [side].[PasswordResetId]
END