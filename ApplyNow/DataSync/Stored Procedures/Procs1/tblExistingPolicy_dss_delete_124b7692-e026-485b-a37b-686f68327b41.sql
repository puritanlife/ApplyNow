CREATE PROCEDURE [DataSync].[tblExistingPolicy_dss_delete_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
SET @sync_row_count = 0; DELETE [dbo].[tblExistingPolicy] FROM [dbo].[tblExistingPolicy] [base] LEFT JOIN [DataSync].[tblExistingPolicy_dss_tracking] [side] ON [base].[ExistingPolicyID] = [side].[ExistingPolicyID] WHERE ((CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL AND [side].[local_update_peer_timestamp] > @marker_local_update_peer_timestamp 
