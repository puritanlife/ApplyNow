CREATE PROCEDURE [DataSync].[tblAdminTempPass_dss_bulkdelete_ac07f40e-d7b8-425c-bf97-76657090c6d0]
...
DELETE [dbo].[tblAdminTempPass] 
OUTPUT DELETED.[AdminTempPassID] INTO @changed FROM [dbo].[tblAdminTempPass] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblAdminTempPass_dss_tracking] t ON p.[AdminTempPassID] = t.[AdminTempPassID]) as changes ON changes.[AdminTempPassID] = base.[AdminTempPassID] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
