CREATE PROCEDURE [DataSync].[tblLeadsNonLicenseStates_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[LeadsNonLicenseStatesID]
ELSE [base].[LeadsNonLicenseStatesID] END) 
as [LeadsNonLicenseStatesID], 
[base].[ProductPlanPeriodID], [base].[UUID], [base].[StateID], [base].[Premium], [base].[StateLicenseTypeID], [base].[Email], [base].[CreatedDate], [base].[LastModifiedDate], [base].[LeadStatus], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblLeadsNonLicenseStates] WHERE [LeadsNonLicenseStatesID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblLeadsNonLicenseStates_dss_tracking] WHERE [LeadsNonLicenseStatesID] = @P_1) [side] ON [base].[LeadsNonLicenseStatesID] = [side].[LeadsNonLicenseStatesID]
END