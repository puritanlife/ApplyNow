CREATE PROCEDURE [DataSync].[tblAddress_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[AddressID]
ELSE [base].[AddressID] END) 
as [AddressID], 
[base].[PersonID], [base].[AddressTypeID], [base].[Address1], [base].[Address2], [base].[City], [base].[StateID], [base].[ZipCode], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblAddress] WHERE [AddressID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblAddress_dss_tracking] WHERE [AddressID] = @P_1) [side] ON [base].[AddressID] = [side].[AddressID]
END