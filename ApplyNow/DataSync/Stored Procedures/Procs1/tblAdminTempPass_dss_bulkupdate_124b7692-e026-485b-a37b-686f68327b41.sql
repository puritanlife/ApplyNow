CREATE PROCEDURE [DataSync].[tblAdminTempPass_dss_bulkupdate_124b7692-e026-485b-a37b-686f68327b41]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [DataSync].[tblAdminTempPass_dss_BulkType_124b7692-e026-485b-a37b-686f68327b41] READONLY
AS
BEGIN
...
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [DataSync].[tblAdminTempPass_dss_tracking] t ON p.[AdminTempPassID] = t.[AdminTempPassID]) as changes ON changes.[AdminTempPassID] = base.[AdminTempPassID]
...
OR (changes.update_scope_local_id IS NULL AND changes.scope_update_peer_key IS NULL AND changes.local_update_peer_timestamp IS NULL) 
 THEN
UPDATE SET [AdminUserID] = changes.[AdminUserID], [Pass] = changes.[Pass], [PasswordUsed] = changes.[PasswordUsed], [ExpirationDate] = changes.[ExpirationDate], [PasswordUsedDate] = changes.[PasswordUsedDate], [CreatedDate] = changes.[CreatedDate], [LastModifiedDate] = changes.[LastModifiedDate]
