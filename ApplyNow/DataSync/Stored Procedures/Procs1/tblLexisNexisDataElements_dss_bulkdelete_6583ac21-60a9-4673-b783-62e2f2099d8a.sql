CREATE PROCEDURE [DataSync].[tblLexisNexisDataElements_dss_bulkdelete_6583ac21-60a9-4673-b783-62e2f2099d8a]
...
DELETE [dbo].[tblLexisNexisDataElements] 
OUTPUT DELETED.[LexisNexisDataElementsID] INTO @changed FROM [dbo].[tblLexisNexisDataElements] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblLexisNexisDataElements_dss_tracking] t ON p.[LexisNexisDataElementsID] = t.[LexisNexisDataElementsID]) as changes ON changes.[LexisNexisDataElementsID] = base.[LexisNexisDataElementsID] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
