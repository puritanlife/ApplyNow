CREATE PROCEDURE [DataSync].[tblDocuSignDataElements_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[DocuSignDataElementsID]
ELSE [base].[DocuSignDataElementsID] END) 
as [DocuSignDataElementsID], 
[base].[ApplyNowID], [base].[Signature], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblDocuSignDataElements] WHERE [DocuSignDataElementsID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblDocuSignDataElements_dss_tracking] WHERE [DocuSignDataElementsID] = @P_1) [side] ON [base].[DocuSignDataElementsID] = [side].[DocuSignDataElementsID]
END