CREATE PROCEDURE [DataSync].[tblPersonPhone_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[PersonPhoneID]
ELSE [base].[PersonPhoneID] END) 
as [PersonPhoneID], 
[base].[PersonID], [base].[PhoneTypeID], [base].[PhoneNumber], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPersonPhone] WHERE [PersonPhoneID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPersonPhone_dss_tracking] WHERE [PersonPhoneID] = @P_1) [side] ON [base].[PersonPhoneID] = [side].[PersonPhoneID]
END