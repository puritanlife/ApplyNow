CREATE PROCEDURE [DataSync].[tblEmailNotification_dss_bulkdelete_124b7692-e026-485b-a37b-686f68327b41]
...
DELETE [dbo].[tblEmailNotification] 
OUTPUT DELETED.[EmailNotificationID] INTO @changed FROM [dbo].[tblEmailNotification] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblEmailNotification_dss_tracking] t ON p.[EmailNotificationID] = t.[EmailNotificationID]) as changes ON changes.[EmailNotificationID] = base.[EmailNotificationID] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
