CREATE PROCEDURE [DataSync].[tblAddressType_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[AddressTypeID]
ELSE [base].[AddressTypeID] END) 
as [AddressTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblAddressType] WHERE [AddressTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblAddressType_dss_tracking] WHERE [AddressTypeID] = @P_1) [side] ON [base].[AddressTypeID] = [side].[AddressTypeID]
END