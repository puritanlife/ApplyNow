CREATE PROCEDURE [DataSync].[tblLoginAttempt_dss_delete_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
SET @sync_row_count = 0; DELETE [dbo].[tblLoginAttempt] FROM [dbo].[tblLoginAttempt] [base] LEFT JOIN [DataSync].[tblLoginAttempt_dss_tracking] [side] ON [base].[LoginAttemptID] = [side].[LoginAttemptID] WHERE ((CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL AND [side].[local_update_peer_timestamp] > @marker_local_update_peer_timestamp 
