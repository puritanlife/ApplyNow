CREATE PROCEDURE [DataSync].[tblPlanType_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[PlanTypeID]
ELSE [base].[PlanTypeID] END) 
as [PlanTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPlanType] WHERE [PlanTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPlanType_dss_tracking] WHERE [PlanTypeID] = @P_1) [side] ON [base].[PlanTypeID] = [side].[PlanTypeID]
END