CREATE PROCEDURE [DataSync].[tblAdminTempPass_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[AdminTempPassID]
ELSE [base].[AdminTempPassID] END) 
as [AdminTempPassID], 
[base].[AdminUserID], [base].[Pass], [base].[PasswordUsed], [base].[ExpirationDate], [base].[PasswordUsedDate], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblAdminTempPass] WHERE [AdminTempPassID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblAdminTempPass_dss_tracking] WHERE [AdminTempPassID] = @P_1) [side] ON [base].[AdminTempPassID] = [side].[AdminTempPassID]
END