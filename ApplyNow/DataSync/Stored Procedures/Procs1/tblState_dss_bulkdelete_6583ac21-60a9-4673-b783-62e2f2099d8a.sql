CREATE PROCEDURE [DataSync].[tblState_dss_bulkdelete_6583ac21-60a9-4673-b783-62e2f2099d8a]
...
DELETE [dbo].[tblState] 
OUTPUT DELETED.[StateID] INTO @changed FROM [dbo].[tblState] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblState_dss_tracking] t ON p.[StateID] = t.[StateID]) as changes ON changes.[StateID] = base.[StateID] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
