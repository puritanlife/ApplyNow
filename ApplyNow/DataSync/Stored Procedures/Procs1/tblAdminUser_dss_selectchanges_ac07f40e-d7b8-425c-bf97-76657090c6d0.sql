CREATE PROCEDURE [DataSync].[tblAdminUser_dss_selectchanges_ac07f40e-d7b8-425c-bf97-76657090c6d0]
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM [dbo].[tblAdminUser] [base] FULL OUTER JOIN [DataSync].[tblAdminUser_dss_tracking] [side] ON [base].[AdminUserId] = [side].[AdminUserId]
 WHERE 
