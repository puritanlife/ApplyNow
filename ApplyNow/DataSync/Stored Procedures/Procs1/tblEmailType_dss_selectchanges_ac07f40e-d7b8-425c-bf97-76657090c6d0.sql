CREATE PROCEDURE [DataSync].[tblEmailType_dss_selectchanges_ac07f40e-d7b8-425c-bf97-76657090c6d0]
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM [dbo].[tblEmailType] [base] FULL OUTER JOIN [DataSync].[tblEmailType_dss_tracking] [side] ON [base].[EmailTypeID] = [side].[EmailTypeID]
 WHERE 
