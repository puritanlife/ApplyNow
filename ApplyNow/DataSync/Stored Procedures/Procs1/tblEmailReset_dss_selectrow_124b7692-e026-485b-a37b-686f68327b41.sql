CREATE PROCEDURE [DataSync].[tblEmailReset_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[EmailResetID]
ELSE [base].[EmailResetID] END) 
as [EmailResetID], 
[base].[OldEmail], [base].[NewEmail], [base].[Token], [base].[TimeStamp], [base].[UsedToken], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblEmailReset] WHERE [EmailResetID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblEmailReset_dss_tracking] WHERE [EmailResetID] = @P_1) [side] ON [base].[EmailResetID] = [side].[EmailResetID]
END