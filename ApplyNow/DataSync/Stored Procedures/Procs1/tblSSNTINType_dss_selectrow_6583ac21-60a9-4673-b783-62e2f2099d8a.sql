CREATE PROCEDURE [DataSync].[tblSSNTINType_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[SSNTINTypeID]
ELSE [base].[SSNTINTypeID] END) 
as [SSNTINTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblSSNTINType] WHERE [SSNTINTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblSSNTINType_dss_tracking] WHERE [SSNTINTypeID] = @P_1) [side] ON [base].[SSNTINTypeID] = [side].[SSNTINTypeID]
END