CREATE PROCEDURE [DataSync].[tblLogStoredProcedureCalls_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[LogStoredProcedureCallsID]
ELSE [base].[LogStoredProcedureCallsID] END) 
as [LogStoredProcedureCallsID], 
[base].[Procedure], [base].[Message], [base].[Parameters], [base].[Output], [base].[StartSPCall], [base].[EndSPCall], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [maint].[tblLogStoredProcedureCalls] WHERE [LogStoredProcedureCallsID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblLogStoredProcedureCalls_dss_tracking] WHERE [LogStoredProcedureCallsID] = @P_1) [side] ON [base].[LogStoredProcedureCallsID] = [side].[LogStoredProcedureCallsID]
END