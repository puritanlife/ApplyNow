CREATE PROCEDURE [DataSync].[tblExistingPolicy_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[ExistingPolicyID]
ELSE [base].[ExistingPolicyID] END) 
as [ExistingPolicyID], 
[base].[ApplyNowID], [base].[PolicyNumber], [base].[IssueDate], [base].[CarrierID], [base].[ChangePolicies], [base].[ExistingPolicies], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblExistingPolicy] WHERE [ExistingPolicyID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblExistingPolicy_dss_tracking] WHERE [ExistingPolicyID] = @P_1) [side] ON [base].[ExistingPolicyID] = [side].[ExistingPolicyID]
END