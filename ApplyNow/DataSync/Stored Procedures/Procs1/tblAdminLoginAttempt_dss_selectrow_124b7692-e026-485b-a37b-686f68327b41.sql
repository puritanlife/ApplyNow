CREATE PROCEDURE [DataSync].[tblAdminLoginAttempt_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[AdminLoginAttemptID]
ELSE [base].[AdminLoginAttemptID] END) 
as [AdminLoginAttemptID], 
[base].[AdminUserID], [base].[Email], [base].[SuccessFlag], [base].[AttemptDate], [base].[AttemptMessage], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblAdminLoginAttempt] WHERE [AdminLoginAttemptID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblAdminLoginAttempt_dss_tracking] WHERE [AdminLoginAttemptID] = @P_1) [side] ON [base].[AdminLoginAttemptID] = [side].[AdminLoginAttemptID]
END