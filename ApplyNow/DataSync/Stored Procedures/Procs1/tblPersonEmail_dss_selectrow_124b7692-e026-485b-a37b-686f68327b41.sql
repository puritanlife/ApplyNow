CREATE PROCEDURE [DataSync].[tblPersonEmail_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[PersonEmailID]
ELSE [base].[PersonEmailID] END) 
as [PersonEmailID], 
[base].[PersonID], [base].[EmailTypeID], [base].[EmailAddress], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPersonEmail] WHERE [PersonEmailID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPersonEmail_dss_tracking] WHERE [PersonEmailID] = @P_1) [side] ON [base].[PersonEmailID] = [side].[PersonEmailID]
END