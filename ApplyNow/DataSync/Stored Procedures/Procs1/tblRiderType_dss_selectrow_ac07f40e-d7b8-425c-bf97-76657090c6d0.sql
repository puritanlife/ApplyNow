CREATE PROCEDURE [DataSync].[tblRiderType_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[RiderTypeID]
ELSE [base].[RiderTypeID] END) 
as [RiderTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblRiderType] WHERE [RiderTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblRiderType_dss_tracking] WHERE [RiderTypeID] = @P_1) [side] ON [base].[RiderTypeID] = [side].[RiderTypeID]
END