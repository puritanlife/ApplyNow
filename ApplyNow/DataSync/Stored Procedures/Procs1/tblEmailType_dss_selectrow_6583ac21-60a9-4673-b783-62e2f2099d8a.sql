CREATE PROCEDURE [DataSync].[tblEmailType_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[EmailTypeID]
ELSE [base].[EmailTypeID] END) 
as [EmailTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblEmailType] WHERE [EmailTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblEmailType_dss_tracking] WHERE [EmailTypeID] = @P_1) [side] ON [base].[EmailTypeID] = [side].[EmailTypeID]
END