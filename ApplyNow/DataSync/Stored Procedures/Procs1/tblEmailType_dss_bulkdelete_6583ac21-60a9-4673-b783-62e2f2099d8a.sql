CREATE PROCEDURE [DataSync].[tblEmailType_dss_bulkdelete_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [DataSync].[tblEmailType_dss_BulkType_6583ac21-60a9-4673-b783-62e2f2099d8a] READONLY
AS
BEGIN
...
DELETE [dbo].[tblEmailType] 
OUTPUT DELETED.[EmailTypeID] INTO @changed FROM [dbo].[tblEmailType] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblEmailType_dss_tracking] t ON p.[EmailTypeID] = t.[EmailTypeID]) as changes ON changes.[EmailTypeID] = base.[EmailTypeID] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
