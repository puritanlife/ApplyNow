CREATE PROCEDURE [DataSync].[tblProductPlanPeriod_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[ProductPlanPeriodID]
ELSE [base].[ProductPlanPeriodID] END) 
as [ProductPlanPeriodID], 
[base].[ProductTypeID], [base].[PlanTypeID], [base].[PeriodTypeID], [base].[PremMin], [base].[PremMax], [base].[CreditingRate], [base].[EffectiveDate], [base].[EndDate], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblProductPlanPeriod] WHERE [ProductPlanPeriodID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblProductPlanPeriod_dss_tracking] WHERE [ProductPlanPeriodID] = @P_1) [side] ON [base].[ProductPlanPeriodID] = [side].[ProductPlanPeriodID]
END