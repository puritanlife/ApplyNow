CREATE PROCEDURE [DataSync].[tblPeriodType_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[PeriodTypeID]
ELSE [base].[PeriodTypeID] END) 
as [PeriodTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPeriodType] WHERE [PeriodTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPeriodType_dss_tracking] WHERE [PeriodTypeID] = @P_1) [side] ON [base].[PeriodTypeID] = [side].[PeriodTypeID]
END