CREATE PROCEDURE [DataSync].[tblPhoneType_dss_bulkdelete_124b7692-e026-485b-a37b-686f68327b41]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [DataSync].[tblPhoneType_dss_BulkType_124b7692-e026-485b-a37b-686f68327b41] READONLY
AS
BEGIN
...
DELETE [dbo].[tblPhoneType] 
OUTPUT DELETED.[PhoneTypeID] INTO @changed FROM [dbo].[tblPhoneType] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblPhoneType_dss_tracking] t ON p.[PhoneTypeID] = t.[PhoneTypeID]) as changes ON changes.[PhoneTypeID] = base.[PhoneTypeID] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
