CREATE PROCEDURE [DataSync].[tblApplyNow_dss_bulkdelete_ac07f40e-d7b8-425c-bf97-76657090c6d0]
...
DELETE [dbo].[tblApplyNow] 
OUTPUT DELETED.[ApplyNowID] INTO @changed FROM [dbo].[tblApplyNow] base JOIN
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p  LEFT JOIN [DataSync].[tblApplyNow_dss_tracking] t ON p.[ApplyNowID] = t.[ApplyNowID]) as changes ON changes.[ApplyNowID] = base.[ApplyNowID] WHERE (changes.update_scope_local_id = @sync_scope_local_id AND changes.scope_update_peer_key = changes.sync_update_peer_key) OR changes.local_update_peer_timestamp <= @sync_min_timestamp
