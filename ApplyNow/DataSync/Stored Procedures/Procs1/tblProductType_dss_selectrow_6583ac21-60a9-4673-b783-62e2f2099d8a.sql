CREATE PROCEDURE [DataSync].[tblProductType_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[ProductTypeID]
ELSE [base].[ProductTypeID] END) 
as [ProductTypeID], 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblProductType] WHERE [ProductTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblProductType_dss_tracking] WHERE [ProductTypeID] = @P_1) [side] ON [base].[ProductTypeID] = [side].[ProductTypeID]
END