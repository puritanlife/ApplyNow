CREATE PROCEDURE [DataSync].[tblPlaidStripeDataElements_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[PlaidStripeDataElementsID]
ELSE [base].[PlaidStripeDataElementsID] END) 
as [PlaidStripeDataElementsID], 
[base].[ApplyNowID], [base].[Charged], [base].[PlaidStripeID], [base].[PaymentAmount], [base].[PaymentMethodTypeID], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPlaidStripeDataElements] WHERE [PlaidStripeDataElementsID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPlaidStripeDataElements_dss_tracking] WHERE [PlaidStripeDataElementsID] = @P_1) [side] ON [base].[PlaidStripeDataElementsID] = [side].[PlaidStripeDataElementsID]
END