CREATE PROCEDURE [DataSync].[tblPerson_dss_selectrow_6583ac21-60a9-4673-b783-62e2f2099d8a]
	@P_1 Int,
...
THEN [side].[PersonID]
ELSE [base].[PersonID] END) 
as [PersonID], 
[base].[BusinessName], [base].[FirstName], [base].[MiddleName], [base].[LastName], [base].[DOB], [base].[ResidentAlien], [base].[USCitizen], [base].[GenderTypeID], [base].[SSNTINTypeID], [base].[BirthStateID], [base].[IDNumber], [base].[SSNTIN], [base].[UUID], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPerson] WHERE [PersonID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPerson_dss_tracking] WHERE [PersonID] = @P_1) [side] ON [base].[PersonID] = [side].[PersonID]
END