CREATE PROCEDURE [DataSync].[tblPPPWithRider_dss_selectrow_124b7692-e026-485b-a37b-686f68327b41]
	@P_1 Int,
...
THEN [side].[PPPWithRiderID]
ELSE [base].[PPPWithRiderID] END) 
as [PPPWithRiderID], 
[base].[ProductPlanPeriodID], [base].[RiderTypeID], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblPPPWithRider] WHERE [PPPWithRiderID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblPPPWithRider_dss_tracking] WHERE [PPPWithRiderID] = @P_1) [side] ON [base].[PPPWithRiderID] = [side].[PPPWithRiderID]
END