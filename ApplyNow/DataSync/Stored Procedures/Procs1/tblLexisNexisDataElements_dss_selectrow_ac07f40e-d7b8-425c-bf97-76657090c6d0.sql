CREATE PROCEDURE [DataSync].[tblLexisNexisDataElements_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[LexisNexisDataElementsID]
ELSE [base].[LexisNexisDataElementsID] END) 
as [LexisNexisDataElementsID], 
[base].[LexisNexisCallTypeID], [base].[ApplyNowID], [base].[ConversationID], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblLexisNexisDataElements] WHERE [LexisNexisDataElementsID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblLexisNexisDataElements_dss_tracking] WHERE [LexisNexisDataElementsID] = @P_1) [side] ON [base].[LexisNexisDataElementsID] = [side].[LexisNexisDataElementsID]
END