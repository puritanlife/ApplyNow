CREATE PROCEDURE [DataSync].[tblRelationshipType_dss_selectrow_ac07f40e-d7b8-425c-bf97-76657090c6d0]
	@P_1 Int,
...
THEN [side].[RelationshipTypeID]
ELSE [base].[RelationshipTypeID] END) 
as [RelationshipTypeID], 
[base].[Descr], [base].[GenderTypeID], [base].[CreatedDate], [base].[LastModifiedDate], (CASE WHEN [side].[local_create_peer_timestamp] IS NOT NULL 
...
ELSE @marker_scope_create_peer_key
 END) 
 end as sync_create_peer_key FROM (SELECT * FROM [dbo].[tblRelationshipType] WHERE [RelationshipTypeID] = @P_1) [base] FULL OUTER JOIN (SELECT * FROM [DataSync].[tblRelationshipType_dss_tracking] WHERE [RelationshipTypeID] = @P_1) [side] ON [base].[RelationshipTypeID] = [side].[RelationshipTypeID]
END